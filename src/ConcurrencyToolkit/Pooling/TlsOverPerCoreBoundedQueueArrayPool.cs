...
/// </remarks>
internal sealed partial class TlsOverPerCoreBoundedQueueArrayPool<T> : ArrayPool<T>
{
  ...
  private static readonly int MaxBuffersPerArraySizePerCore = Settings.MaxBuffersPerArraySizePerCore;
  ...
  /// </summary>
  private readonly PerCoreQueues?[] _buckets = new PerCoreQueues[NumBuckets];
  ...
  private PerCoreQueues CreatePerCoreLockedStacks(int bucketIndex)
  {
    var inst = new PerCoreQueues();
  ...
  {
    ConcurrencyToolkitEventSource log = ConcurrencyToolkitEventSource.Log;
    T[]? buffer;
    ...
    PerCoreQueues?[] perCoreBuckets = _buckets;
    if ((uint)bucketIndex < (uint)perCoreBuckets.Length)
    {
      PerCoreQueues? b = perCoreBuckets[bucketIndex];
      if (b is not null)
      {
        buffer = b.TryPop();
        if (buffer is not null)
    ...
    if (log.IsEnabled())
    {
      ...
      log.BufferRented(bufferId, buffer.Length, Id, ConcurrencyToolkitEventSource.NoBucketId);
      log.BufferAllocated(bufferId, buffer.Length, Id, ConcurrencyToolkitEventSource.NoBucketId,
        bucketIndex >= _buckets.Length
          ? ConcurrencyToolkitEventSource.BufferAllocatedReason.OverMaximumSize
          : ConcurrencyToolkitEventSource.BufferAllocatedReason.PoolExhausted);
    }
  ...
  public override void Return(T[] array, bool clearArray = false)
  {
    ...
    if ((uint)bucketIndex < (uint)tlsBuckets.Length)
    {
      ...
      if (prev is not null)
      {
        PerCoreQueues queueBucket = _buckets[bucketIndex] ?? CreatePerCoreLockedStacks(bucketIndex);
        returned = queueBucket.TryPush(prev);
      }
    }
    ...
    if (log.IsEnabled() && array.Length != 0)
    {
      ...
      if (!(haveBucket & returned))
      {
        log.BufferDropped(array.GetHashCode(), array.Length, Id,
          haveBucket ? bucketIndex : ConcurrencyToolkitEventSource.NoBucketId,
          haveBucket
            ? ConcurrencyToolkitEventSource.BufferDroppedReason.Full
            : ConcurrencyToolkitEventSource.BufferDroppedReason.OverMaximumSize);
      }
    }
  }
  ...
  private ThreadLocalArray[] InitializeTlsBucketsAndTrimming()
  {
    ...
    {
      Gen2GcCallback.Register(s => ((TlsOverPerCoreBoundedQueueArrayPool<T>)s).Trim(), this);
    }
  ...
  private sealed class PerCoreQueues
  {
    ...
    private readonly BoundedQueueWrapper[] _perCoreStacks;

    /// <summary>Initializes the stacks.</summary>
    public PerCoreQueues()
    {
      // Create the stacks.  We create as many as there are processors, limited by our max.
      var stacks = new BoundedQueueWrapper[s_lockedStackCount];
      for (int i = 0; i < stacks.Length; i++)
      {
        stacks[i] = new BoundedQueueWrapper();
      }
    ...
    public bool TryPush(T[] array)
    {
      ...
      BoundedQueueWrapper[] stacks = _perCoreStacks;
  ...
  private sealed class BoundedQueueWrapper
  {
    /// <summary>The bounded queue.</summary>
    private readonly UnpaddedFixedSizeConcurrentQueue<T[]> _queue = new(MaxBuffersPerArraySizePerCore);

    /// <summary>Timestamp set by Trim when it sees this as 0.</summary>
    private int _millisecondsTimestamp;

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public bool TryPush(T[] array) => _queue.TryEnqueue(array);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public T[]? TryPop() => _queue.TryDequeue(out var arr) ? arr : null;
    ...
    public void Trim(int currentMilliseconds, int id, Utilities.MemoryPressure pressure, int bucketSize)
    {
      ...
      int StackHighTrimCount = MaxBuffersPerArraySizePerCore; // Trim all items when pressure is high
      const int
        StackLargeBucket = 16384; // If the bucket is larger than this we'll trim an extra when under high pressure
      const int StackModerateTypeSize = 16; // If T is larger than this we'll trim an extra when under high pressure
      const int
        StackLargeTypeSize = 32; // If T is larger than this we'll trim an extra (additional) when under high pressure
      ...
      bool dequeued;
      while ((dequeued = _queue.TryDequeue(out var array)) && trimCount-- > 0)
      {
      ...
      _millisecondsTimestamp = dequeued
