...
/// </summary>
/// <remarks>
/// Head and Tail indices are unpadded, because it doesn't matter for pooling:
///  - there are typically one <see cref="UnpaddedFixedSizeConcurrentQueue{T}"/> per processor core to avoid contentions, but it multiplies padding size
///  - producers and consumers are the same threads
///  - slots count may be small, and size of slots array may be comparable with the size of paddings
/// </remarks>
[DebuggerDisplay("Capacity = {Capacity}")]
public sealed class UnpaddedFixedSizeConcurrentQueue<T> : IProducerConsumerCollection<T>
{
  ...
  /// <summary>The head and tail positions.</summary>
  /// <remarks>Dequeuing happens from the head, enqueuing happens at the tail.</remarks>
  private HeadAndTail _headAndTail; // mutable struct: do not make this readonly

  /// <summary>Creates the bounded concurrent queue.</summary>
  /// <param name="boundedLength">
  /// The maximum number of elements the segment can contain.  Must be a power of 2.
  /// </param>
  public UnpaddedFixedSizeConcurrentQueue(int boundedLength)
  {
    // Validate the length
    if (boundedLength < 2)
